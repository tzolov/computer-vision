...
NOTE: The link:../spring-cloud-starter-stream-processor-semantic-segmentation-cv[semantic-segmentation-cv] provides an equivalent semantic segmentation
implementation but uses https://github.com/bytedeco/javacv[JavaCV/OpenCV API] for implementing the image pre/post processing tasks.
